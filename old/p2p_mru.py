...
parser.add_argument("--generator", default="mru", choices=["res", "ir", "ed", "mru", "sa", "sa_I"])
parser.add_argument("--discriminator", default="conv", choices=["res", "ir", "conv", "mru", "sa", "sa_I"])
...
parser.add_argument("--channel_fac", default=16, type=int, help="faction of channel in self attention modual")
parser.add_argument("--zoom_scale", type=float, default=1.0, help="zoom_scale of edge maps")
parser.add_argument("--rotate_angle", type=float, default=0.0, help="rotate_angle of edge maps, in degree")
...
def transform(image):
    ...
    size = math.floor(a.scale_size*a.zoom_scale)
    r = tf.image.resize_images(r, [size, size], method=tf.image.ResizeMethod.AREA)
    
    if not a.rotate_angle < 1e-5:
        r = tf.contrib.image.rotate(r, a.rotate_angle/180.0*3.14159)
        # r = tf.image.rot90(r)
        
    if a.zoom_scale < 1.0:     
        off_set_floor = math.floor((a.scale_size - size) / 2)
        off_set_ceil = math.ceil((a.scale_size - size) / 2)           
        paddings = tf.constant([[off_set_floor, off_set_ceil],[off_set_floor, off_set_ceil], [0,0]])
        r = tf.pad(r, paddings, mode="REFLECT")
        print(r.get_shape())
    if 1:
        offset=[60,40]
        pad = 50
        ...
        offset = tf.cast(tf.floor(tf.random_uniform([2], 0, size - CROP_SIZE + 1, seed=seed)), dtype=tf.int32)
        #center_offset = math.floor((size - a.scale_size) / 2)
        if size > CROP_SIZE:
            r = tf.image.crop_to_bounding_box(r, offset[0], offset[1], CROP_SIZE, CROP_SIZE)
            #r = tf.image.crop_to_bounding_box(r, center_offset, center_offset, CROP_SIZE, CROP_SIZE)
...
def parse_function_test(example_proto):
    ...
    edge = tf.decode_raw(parsed_features['edge'], tf.float64) 
    ...
    if a.input_type == "df":
        return photo, df, filenames  
    elif a.input_type == "edge": 
        return photo, edge, filenames
...
def parse_function(example_proto):
    ...
    #edge = center_crop_resize(tf.image.grayscale_to_rgb(edge))
    #df = center_crop_resize(tf.image.grayscale_to_rgb(df))
    #photo = center_crop_resize(photo)  
...
def read_tfrecord():
    ...
    tfrecord_fn = a.input_dir
    dataset = tf.contrib.data.TFRecordDataset(tfrecord_fn)
    dataset = dataset.map(parse_function_test)  # Parse the record into tensors.
...
def save_images(fetches, step=None):
    ...
    if a.load_tfrecord == True:
        for i, fn in enumerate(fetches["filenames"]):
            ...
            # YuhangLi: if you want to add "inputs" and "targets" in kinds, please mkdir of them
            for kind in ["outputs"]:
                filename = name + ".png"
...
def main():
    ...
    if a.generator=='sa' or a.generator=='sa_I':
        with tf.name_scope("convert_beta"):
            converted_beta = tf.image.convert_image_dtype(model.beta_list[-1], dtype=tf.uint8, saturate=True)
