...
parser.add_argument("--trace_freq", type=int, default=1000, help="trace execution every trace_freq steps")
...
def save_images(fetches, step=None):
    ...
    if a.load_tfrecord == True:
        for i, _ in enumerate(fetches["inputs"]):
            ...
            # YuhangLi: only save the outputs
            for kind in ["outputs", "beta"]:
    ...
    with tf.name_scope("encode_images"):
        if a.load_tfrecord:
            display_fetches = {
                ...
                "beta": tf.map_fn(tf.image.encode_png, converted_beta, dtype=tf.string, name="beta_pngs"),
            }
        else:
            display_fetches = {
                "paths": examples.paths,
