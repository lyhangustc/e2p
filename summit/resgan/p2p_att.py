...
parser.add_argument("--channel_fac", default=32, type=int, help="faction of channel in self attention modual. Set to large to save GPU memory")
...
Model = collections.namedtuple("Model", "outputs, beta_list, predict_real, predict_fake, discrim_loss, discrim_grads_and_vars, gen_loss_GAN, gen_loss_L1, gen_loss_fm, gen_grads_and_vars, train, train_attention")
...
def create_model(inputs, targets):
    ...
    with tf.device("/gpu:0"):
        ...
        with tf.name_scope("generator_train_attention"):
            with tf.control_dependencies([discrim_train]):
                gen_tvars_attention =  tf.get_collection(tf.GraphKeys.TRAINABLE_VARIABLES, scope='generator/decoder/decoder_1') + \
                    tf.get_collection(tf.GraphKeys.TRAINABLE_VARIABLES, scope='generator/decoder/decoder_2') + \
                    tf.get_collection(tf.GraphKeys.TRAINABLE_VARIABLES, scope='generator/decoder/attention_0')
                gen_optim_attention = tf.train.AdamOptimizer(a.lr_gen, a.beta1)
                gen_grads_and_vars_attention = gen_optim.compute_gradients(gen_loss, var_list=gen_tvars, colocate_gradients_with_ops=True)
                gen_train_attention = gen_optim.apply_gradients(gen_grads_and_vars)
    ...
    return Model(
        ...
        train_attention=tf.group(update_losses, incr_global_step, gen_train_attention),
    )
...
def main():
    ...
    restore_var = tf.get_collection(tf.GraphKeys.TRAINABLE_VARIABLES, scope='generator/encoder') + \
                tf.get_collection(tf.GraphKeys.TRAINABLE_VARIABLES, scope='generator/middle') + \
                tf.get_collection(tf.GraphKeys.TRAINABLE_VARIABLES, scope='generator/decoder/decoder_0') + \
                tf.get_collection(tf.GraphKeys.TRAINABLE_VARIABLES, scope='discriminator_global') + \
                tf.get_collection(tf.GraphKeys.TRAINABLE_VARIABLES, scope='discriminator_patch')
